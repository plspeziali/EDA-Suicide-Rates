...
distribuzioni di tassi di suicidi senza differenze significative. Andiamo ora ad analizzare
...
\begin{table}[htbp]
    ...
    \end{center}
\end{table}
Possiamo osservare come solo nel secondo e quarto gruppo il p-value supera il valore di 0.05
...
le differenze tra i vari anni siano generalizzate e non riguardino sono signole coppie 
\begin{table}[htbp]
...
Visti i risultati del test di Friedman, vediamo se l'ANOVA a misure ripetute li appoggia
o meno.
In Tab.~\ref{tab7} 
\begin{table}[htbp]
    ...
    \end{center}
\end{table}
Vediamo come per entrambi i gruppi l'ANOVA ci dia un p-value superiore allo 0.05 (seppure molto
vicino nel caso del primo gruppo), se avessimo avuto tutte e tre le supposizioni verificate
ciò ci avrebbe portato ad accettare l'ipotesi nulla e quindi dire che nei diversi gruppi
abbiamo le stesse distribuzioni, tuttavia, per quanto detto prima, questi risultati non sono
significativi o utili ai fini della nostra analisi.

\section{Conclusioni}

Dall'analisi appena presentata si può ricavare in conclusione che è arduo
affermare esistano paesi che nel corso degli anni vadano ad avere distribuzioni
del tassi di suicidio statisticamente simili e, anche quando accade,
rimane sempre il dubbio se ciò sia casuale e un cambio di singolo paese o intervallo
temporale possa far cadere tutte le nostre supposizioni o se ci sia effettivamente una
causalità sotto a livello sociale. Quel che possiamo dire per ora è che in certi
casi, molto specifici, differenze significative non vengono riscontrate mentre in
altrettanti casi, molto specifici anch'essi, tali differenze esistono.
Il perché le sopracitate differenze esistano o meno in questi determinati frangenti
rendono i risultati di questo studio una base per possibili studi futuri.
